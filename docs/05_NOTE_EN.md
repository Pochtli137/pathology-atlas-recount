# Reproduction note · draft, 23 September 2026 · not peer reviewed
  ...
  11 of 21cohorts where the number of “prognostic” genes is within what chance alone produces
...
Section 2 gives the mean. Genes are correlated, so the count in a single dataset scatters far more than a binomial would. We permuted survival 200 times per cohort, and 2,000 times in the eight cohorts where the first 200 put the answer between 0.01 and 0.2 (BLCA, BRCA, CESC, COAD, GBM, HNSC, READ, STAD), and ran the procedure on all genes each time.
Figure 2Genes below p < 0.001 in a permuted dataset (bar: median, line: 5th to 95th percentile of 200 permutations, 2,000 in eight cohorts) as a share of the genes HPA labels prognostic in that cohort. Above 100 % the labelled count is within chance. TGCT (22 labelled genes) runs off the axis and is clipped at the top.

In a typical permuted dataset 100 to 190 genes come out “prognostic” (median). In one dataset in twenty it is 450 to 870. The worst single draw gave 3,900 in THCA and 3,163 in KIRC (of 200 each) and 3,008 in HNSC (of 2,000).
In eleven of twenty-one cohorts the observed number of prognostic genes is within what permutation produces (one-sided permutation p > 0.05 for the count): BLCA, COAD, GBM, LUSC, OV, PRAD, SKCM, STAD, TGCT, THCA, UCEC. BLCA and READ were borderline at 200 permutations; at 2,000, BLCA gives p = 0.062 and READ 0.042, each with a 95 % Monte Carlo interval of about ±0.01. The ten remaining cohorts exceed chance at 0.05, but that is 21 tests. With Benjamini–Hochberg at 5 % across the 21 cohorts seven remain: KIRC, KIRP, LIHC, LUAD, KICH, CESC and PAAD (q 0.035 to 0.045). HNSC (q 0.053), BRCA (0.064) and READ (0.088) do not. Bonferroni (p < 0.0024) cannot be settled for the six strongest, which had 200 permutations and a resolution of 0.005; in KIRC, KIRP and LIHC the observed count is above the largest of the 200 permuted counts. KICH passes on nine deaths and carries the caution of section 2.
...
The per-gene label on proteinatlas.org separates “validated prognostic” from “potential prognostic”, so a user can see whether a gene replicated. What the page does not show is that “potential prognostic, p < 0.001” carries very different weight by cohort: unadjusted association is present in kidney (KIRC, KIRP), liver, lung adenocarcinoma and pancreatic cancer, weaker in CESC, not established once the 21 cohorts are corrected for in HNSC, BRCA and READ, and not distinguishable from chance at the cohort level in eleven cohorts, where the same label is shown. None of this is prognostic value on top of stage and grade, which neither HPA nor we adjust for.
...
Not done: no adjustment for age, stage, grade or sex (Gilis et al. adjust for age), which is the main threat to every gene count here; Breslow ties instead of R’s Efron; the authors’ R code was read, not run; the whole-cohort test in the six strongest cohorts has 200 permutations, too few to settle Bonferroni across 21 cohorts; Altman 1994 and Lausen & Schumacher 1992 were checked against the maxstat source, not read in the original.
